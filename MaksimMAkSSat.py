...
'''
# Взять квадратный корень от числа 16
print(16 ** 0.5)
...
import math as m
print(m.sqrt(16))  # Подключение библиотеки с коротким именем (переменная)

from math import sqrt, ceil, floor  # Подключили конкретные функции из библиотеки
print(sqrt(16))
...
# Сочетание клавиш ctrl + B показывает описание библиотек и функций 
'''
...
# Среди натуральных чисел, не превышающих 10**8, найдите все числа, соответствующие
# маске 1*23??56, делящиеся на 171 без остатка.
...
import math as m
print(m.dist([4, 5], [6, 7]))  # 2.8284271247461903
print(m.ceil(7/2))  # 4
print(m.floor(7/2))  # 3
'''
...
print(M[:3])  # ['a', 'b', 'c'] - Первые три элемента
...
print(M[1:-1])  # ['b', 'c', 'd'] - не берем первый и последний элемент
...
print([x for x in range(5, 10)])
print([x for x in range(0, 10+1, 2)])
print([x for x in range(0, 10+1) if x % 2 == 0])
print([x ** 2 for x in range(0, 10+1) if x % 2 == 0])

from random import randint
N = [randint(0, 100) for i in range(randint(5, 15))]
print(N)
copied1 = [x for x in N if N.count(x) == 1]
copied2 = [x for x in N if N.count(x) == 2]
copied3 = [x for x in N if N.count(x) == 3]

chet = [x for x in N if x % 2 == 0]
nechet = [x for x in N if x % 2 != 0]

A = [x for x in N if x % 100 == 15]
A = [x for x in N if len(str(abs(x))) == 2]
