...
A = [x for x in M if x % 100 == 17]  #Список элементов, оканчивающихся на 17
B = [x for x in M if len(str(abs(x))) == 4]  #Список четырёхзначных элементов
for i in range(len(M)-2):
    x, y, z = M[i], M[i+1], M[i+2]
    if (x in B) + (y in B) + (z in B) == 2: #Проверяем, чтобы ровно 2 числа были четырёхзначными
        if (int(x % 5 == 0) + int(y % 5 == 0) + int(z % 5 == 0)) >= 1: #Проверяем, чтобы хотя бы одно число из тройки делилось на 5(поэтому >= 1)
            if (x + y + z) > max(A): # Проверяем, чтобы сумма элементов тройки была больше максимального элемента, оканчивающегося на 17
                R.append(x+y+z) # Добавляем все подходящие суммы элементов троек в список
print(len(R), max(R)) # Выводим ответ: 21, 114132(но в поле для ответа запишем 21114132)
