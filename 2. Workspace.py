...
# является 49, при этом траектория вычислений содержит 18 или 30,
# и не содержит чисел 28 и 36?

def F(x,k):
    if x == k:
        return 1
    if x > k or x == 28 and x== 36:
        return 0
    return F(x+1,k) + F(x+5,k) + F(x*3,k)

print(F(2, 18) * F(18, 30) * F(30, 49))

def F(x, k):
    if x == k:
        return 1
    if x > k or x == 28 or x == 36:
        return 0
    return F(x+1,k) + F(x+5,k) + F(x*3,k)
